...
!(async function({Array: _, console: t, customElements: s, CustomEvent: o, document: e, fetch: a, HTMLElement: l, localStorage: h, location: n, Object: u, navigator: i, setTimeout: r, URLSearchParams: d, Promise: c} = window) {
    ...
    const i_ = _ => {
        ...
        Exodus_20_15_Thou_shalt_not_steal___Y: {
            ...
            css: `{name}{user-select:none;grid-area:left;display:flex;flex-direction:column}.${n_} {display:flex;flex:1;align-items:center;justify-content:center}.${n_} {opacity:.5;margin-bottom:0em;border-radius:var(--radius);background:lightblue;cursor:pointer}.${h_} {opacity:1;background:var(--chaptertitle);color:var(--textcolor)}` + x_
        },
    ...
    s[g](T_.AREA51.name, class extends b_ {
        ...
        _EVENT_DISCLAIMER_() {
            ...
            _.setHTMLUnsafe('\n          <style>dialog{background:beige;font:21px arial}dialog::backdrop{background:rgba(0, 0, 0, 0.4)}</style>\n          <form method="dialog"><h3>Disclaimer</h3><p>Code, Story and Translations done by AI</p><button type="submit">Close</button></form>'), 
            _.showModal(), _[N] = t => {
                t.target === _ && _.close();
            };
        }
